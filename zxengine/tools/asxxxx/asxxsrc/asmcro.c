 ...
 *		.irp			define an inline indefinite repeat macro
 *by arguments .irpc			define an inline indefinite repeat macro
 *by characters .rept			define an inline repeating macro .mexit
 *exit to end of macro .endm			end of macro .nchr
 *assign number of characters in a string to a symbol .narg
 *assign number of expansion arguments to a symbol .ntyp
 *assign 0/1 if absolute/relocatable symbol .nval			assign
 *value of argument to an absolute symbol .mdelete		delete a macro
 *definition
 *
 ...
 *		char 	id[]		id string
 *		mne    *mp		pointer to an assembler mnemonic
 *structure macrofp *nfp		macro psuedo FILE Handle mcrdef *nq
 *pointer to a macro definition structure mcrdef *np		pointer to a
 *macro definition structure int	rptcnt		repeat count evaluation
...
int code;
{
  ...
  case O_CHECK:
    ...
    while (more()) {
      getid(id, getnb());
      if (((mp = mlookup(id)) != NULL) && (mp->m_type == S_MACRO)) {
        switch (mp->m_valu) {
        case O_MACRO:
        case O_IRP:
        case O_IRPC:
 ...
 *
 *	global variables:
 *		mcrdef *mcrp		pointer to the new macro definition
 *structure
 *
 *	functions called:
 *		mne *	mlookup()	assym.c
 ...
 *					assembler-source text line for
 *processing char *	ip		pointer into the assembler-source text
 *line in ib[] int	flevel		current IF-ELSE-ENDIF level int	tlevel
 *current IF-ELSE-ENDIF level index int	lnlist		current LIST-NLIST flags
...
FILE *fp;
{
  ...
  if (nfp->lstptr == NULL) {
    if (nfp->npexit == 0) {
      if ((flevel != nfp->flevel) || (tlevel != nfp->tlevel)) {
        err('i');
      }
    }
    /*
