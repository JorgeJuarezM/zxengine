 ...
 *structure int	asmline		assembler source file line number asmf *
 *asmp		pointer to first assembler file structure int	aserr
 *assembler error counter int	bflag		-b(b), listing mode flag int
 *cb[]		array of assembler output values int	cbt[]		array of
 *assembler relocation types describing the data in cb[] int *	cp
 *pointer to assembler output array cb[] int *	cpt		pointer to
 *assembler relocation type output array cbt[] time_t	curtim		current
 *time string pointer def *	defp		pointer to a def structure char
 *eb[]		array of generated error codes char *	ep		pointer
 *into error list array eb[] int	fflag		-f(f), relocations
 *flagged flag int	flevel		IF-ELSE-ENDIF flag will be non zero for
 *false conditional case a_uint	fuzz		tracks pass to pass changes in
 *the address of symbols caused by variable length instruction formats int
 *gflag		-g, make undefined symbols global flag int	tflag
 *-h, hidden option for diagnostic info dump char	ib[]
 *assembler-source text line int	ifcnd[]		array of IF statement
 *condition values (0 = FALSE) indexed by tlevel int	iflvl[]		array of
 *IF-ELSE-ENDIF flevel values indexed by tlevel int	incfil		current
 *include file count int	incline		include source file line number
    ...
    if (*p == '-') {
      ...
      if (iflag-- > 0) {
        ...
        if (asmp == NULL) {
          ...
          asmp = (struct asmf *)new (sizeof(struct asmf));
          if (asmo != NULL) {
            /*
          ...
          asmc->next = (struct asmf *)new (sizeof(struct asmf));
          asmc = asmc->next;
        }
        asmc->next = NULL;
        asmc->objtyp = T_ASM;
...
int intsiz() { return (sizeof(INT32)); }
...
{
  if (asmo == NULL) {
    asmo = (struct asmf *)new (sizeof(struct asmf));
    asmc = asmo;
  } else {
    asmc->next = (struct asmf *)new (sizeof(struct asmf));
    asmc = asmc->next;
 ...
 *
 *	global variables:
 *		asmf *	asm 		pointer to current assembler file
 *structure asmf *	asmp		pointer to first assembler file
 *structure FILE *	lfp		list output file handle FILE *	ofp
 *relocation output file handle FILE *	tfp		symbol table output file
 *handle
 *
 *	functions called:
  ...
  case S_CONDITIONAL:
    ...
    if (mp->m_valu < O_IFEND) {
      if (mp->m_valu == O_IF) {
        ...
        getid(&id[3], getnb());
        xp = mlookup(id);
        if ((xp != NULL) && (xp->m_type == S_CONDITIONAL) &&
            (xp->m_valu != O_IF)) {
          mp = xp;
          comma(0);
        } else {
      ...
      if (c == '(') {
        do {
          if ((c = getnb()) == '!') {
            ...
            if (symeq(id, "err", 1)) {
              flags |= LIST_ERR;
            } else if (symeq(id, "loc", 1)) {
              flags |= LIST_LOC;
            } else if (symeq(id, "bin", 1)) {
              flags |= LIST_BIN;
            } else if (symeq(id, "eqt", 1)) {
              flags |= LIST_EQT;
            } else if (symeq(id, "cyc", 1)) {
              flags |= LIST_CYC;
            } else if (symeq(id, "lin", 1)) {
              flags |= LIST_LIN;
            } else if (symeq(id, "src", 1)) {
              flags |= LIST_SRC;
            } else if (symeq(id, "pag", 1)) {
              flags |= LIST_PAG;
            } else if (symeq(id, "lst", 1)) {
              flags |= LIST_LST;
            } else if (symeq(id, "md", 1)) {
              flags |= LIST_MD;
            } else if (symeq(id, "me", 1)) {
              flags |= LIST_ME;
            } else if (symeq(id, "meb", 1)) {
              flags |= LIST_MEB;
            } else {
              err('u');
            }
 ...
 *		a_uint		equtype		equate type
 *(O_EQU,O_LCLEQU,O_GBLEQU)
 *
